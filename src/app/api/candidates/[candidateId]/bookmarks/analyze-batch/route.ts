...
3. **▲と×の個数でランクを機械的に決める**。「迷ったらB」は禁止。懸念を数えて必ず A/B/C のどれかに振り分ける

| ランク | 判定条件 |
|---|---|
| A | 必須要件を全て満たし、選考分析の全項目が〇（▲・×が0個）。年齢・経験年数・転職回数が求人の想定レンジ内 |
| B | 必須要件を満たすが、▲が1個だけある（懸念が1点に絞れる）。×は0個 |
| C | 必須要件は満たすが、▲が2個以上、または（必須要件以外に）×が1個以上ある。または必須要件未充足だが推薦状でカバー可能性あり |
| D | 必須要件の充足が不十分（学歴・資格・経験年数のいずれかが未充足） |

**A判定の条件について:** 過去の通過実績・企業の採用温度感は、判定時に分かっている場合のみ「B→A」「A→B」の補正に使う。Aの必須条件ではない。
この補正が使えるのは **A と B の間だけ** である。▲が2個以上ある／×がある求人（＝C）を「経験の親和性が高い」「致命的ではない」等の理由でBへ引き上げてはならない。個数ルールが常に優先する。

**判定の甘さの目安:** ランク付き求人のうち通過率Bが4割を超えている状態は、懸念を数えずにBへ逃げている可能性が高い。

**【出力順に関する必須手順】** 「■ 通過率」行は出力上は「◆ 選考分析（CA向け）」より前に来るが、**判定の順序は逆である**。1件の求人を書き始める前に、必ず先に「◆ 選考分析（CA向け）」の全項目と各項目の 〇/▲/× を確定させ、その ▲ と × の個数を数えてから通過率ランクを決め、「■ 通過率」行を書くこと。ランクを先に決めてから選考分析の記号を後付けで合わせてはならない。出力し終えた時点で、選考分析に実際に付いた ▲・× の個数と上表の条件が必ず一致していること（不一致は誤りであり、その場合は選考分析の記号側を正として通過率ランクを直す）。
...
- 全体で4〜7項目程度に収める
- **このセクションの ▲・× の個数が「■ 通過率」ランクの根拠である**（評価ルール② の判定手順を参照）。記号は実態に忠実に付け、通過率ランクと必ず整合させること（▲0個・×0個=A / ▲1個・×0個=B / ▲2個以上または×1個以上=C）
