...
## 出力フォーマット
各求人の分析は以下の形式で出力すること。求人と求人の間には必ず空行を2行入れて区切ること：
...
◆ 推薦コメント
1〜2文で総括。最優先で提案すべき/確認すべき/チャレンジとして提案 等の推薦度を明示。

---

- 【会社名】行 → 空行 → ■行3つ → 空行 → ◆セクション の順に必ず改行を入れること
- 各セクション（◆）の間には必ず空行を1行入れること
- 箇条書きは「・」で始め、各項目ごとに改行すること
- 1つの「・」は1〜2文（最大80文字程度）で簡潔にまとめること
- **絶対に長文を改行なしで続けないこと**（200文字以上の改行なし段落は禁止）
- マッチポイントは2〜4個、懸念点は0〜3個程度
- 推薦コメントも長くなる場合は2〜3行に分けること
- 「---」の区切り線で各求人を明確に分離すること
