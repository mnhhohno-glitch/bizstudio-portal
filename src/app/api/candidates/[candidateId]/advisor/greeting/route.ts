...
- 今後の進め方（ネクストアクション）を必ず含めてください。求人の方向性、書類準備、次回面談の内容予告など、具体的な次のステップを記載してください。
- 長さは300〜500文字程度`
      : `メール向けの挨拶文を作成してください。
- 件名を含めてください（「件名：○○」の形式）
- ビジネスメールとして適切な文体
...
4. 【ネクストアクションの整理】
   面談内容やチャット履歴をもとに、今後のアクションを整理して挨拶文に含めてください。
   例:
   - 求人のご提案（どのような方向性の求人を探すか）
   - 書類の準備（職務経歴書の作成・修正など）
   - 次回面談で話す内容の予告
   - 確認事項や宿題（希望条件の整理、必要書類の準備など）

   ネクストアクションは箇条書きではなく、文章の流れの中で自然に組み込んでください。
   「今後の流れ」や「次のステップ」として、求職者が安心して次に進めるように記載してください。
