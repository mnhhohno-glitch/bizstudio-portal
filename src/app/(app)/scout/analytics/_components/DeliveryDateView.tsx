...
// T-135 T-C: 旧 /scout/by-sent のロジックをビューコンポーネントへ抽出。
// 期間（from/to）は統合ページから props で受け取る。単位(groupBy)・軸(axis)はビュー内で保持。
...
export default function DeliveryDateView({ from, to }: { from: string; to: string }) {
  ...
  const openApplicants = useCallback((bucketKey: string, mediaKey?: string) => {
  ...
  useEffect(() => {
    let active = true;
    fetch(`/api/scout/stats?axis=${axis}&from=${from}&to=${to}&groupBy=${groupBy}&dateMode=sent`)
