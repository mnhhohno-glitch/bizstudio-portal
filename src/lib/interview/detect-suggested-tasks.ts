// T-151 Phase 2-3: 面談ログ（Notta 等）から固定3種のタスク約束を検出する。
// （3種目 DOCUMENT_SEND＝応募書類の作成・送付は後日追加。「その他」カテゴリで起票し、
//   タイトルに detail を入れて CA が内容を判別できるようにしている。）
//
...
「CAが求職者に対してこれからやると約束した作業」のうち、下記3種類だけを抽出するアシスタントです。

# 抽出する種別（この3つ以外は絶対に出力しない）
- JOB_SEARCH_SEND: CAが求人を検索・ピックアップして求職者に送る／マイページに掲載する約束
- FORM_SURVEY: CAがGoogleフォーム（アンケート・質問票）を求職者に送付する約束
- DOCUMENT_SEND: CAが求職者に渡す**応募書類**（職務経歴書・履歴書・志望動機を入れた書類など）を
  作成・修正して送る約束。「書類を整えて送ります」「書類を作ってお送りします」等の表現も含む。
  ★求人票の送付は DOCUMENT_SEND ではなく JOB_SEARCH_SEND。書類として二重に出さないこと。
...
- 求職者側のアクション（例:「@山田さん：Googleフォームに追加情報を入力する」
  「@山田さん：履歴書・職務経歴書を記入・返送する」「証明写真を撮って送ってください」）。
  **主語がCAでないものは対象外。** 書類の話でも、作るのが求職者なら出さない。
- 過去に完了した紹介への言及（例:「先日ご紹介させていただいた求人が9件」）。
- **すでに終わっている書類作業**（例:「先週お送りした職務経歴書ですが」のように過去形で
  語られているだけのもの）。これから送る約束が無いなら DOCUMENT_SEND を出さない。
- 上記3種以外のCA側アクション（例: 面接対策資料の準備、次回面談の日程確保、
  マイページの発行のみ）。
...
# DOCUMENT_SEND のときだけ付ける追加項目
- detail: 約束の中身が一目で分かる**日本語の短い語句（20字以内・体言止め）**。
  タスクのタイトルにそのまま入るので、書類の種類と作業が分かるように書く。
  例:「職務経歴書の修正・送付」「志望動機入り書類の作成・送付」「履歴書の作成・送付」
- docAction: "create"（作成・修正を含む場合）または "send"（できている書類を送るだけの場合）。
  作成と送付の両方を含むときは "create" にする。
...
<<<T150_TASKS
{"tasks":[{"kind":"JOB_SEARCH_SEND","due":"this_week"},{"kind":"DOCUMENT_SEND","due":"tomorrow","detail":"職務経歴書の修正・送付","docAction":"create"}]}
T150_TASKS>>>`;
