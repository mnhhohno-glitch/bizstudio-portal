...
// T-XXX step8: 「## 出力フォーマット」節だけを短い書き方に差し替えた（本人向け2〜3文・CA項目は短い一文。
//   ▲・×の項目は省かせない・まとめさせない）。評価ルール（判定部分）は不変のため EVAL_LOGIC_VERSION は上げていない。
//   差し替え前後の比較は scripts/compare-comment-compact-t-xxx.ts・docs/reports/T-XXX_comment-compact.md。
...
/**
 * T-XXX step8: 判定に関わる版番号。変更なしスキップ（eval-history.ts）は、指示文の中身そのものではなく
 * この版番号と SKILL の本文で「前回と同じ判定か」を決める。
 *   - 判定そのものが変わる変更（評価ルールの基準・▲×の数え方・総合評価表など）→ この数字を上げる（全件が評価し直しになる）
 *   - 書き方だけの変更（出力フォーマットの長さ・言い回し・バッチ指示）→ 上げない（既存の評価はそのまま再利用される）
 * SKILL.md（判定基準）の本文は別途キーに含めるので、SKILL の更新では上げなくてよい。
 * 1 = step8 の書き方短縮より前から続く判定（短縮では上げていない）。
 */
export const EVAL_LOGIC_VERSION = 1;

/** 判定の同一性キー（版番号 + SKILL 本文のハッシュ）。job_eval_records.logic_key に保存する。 */
export function evalLogicKey(): string {
  return createHash("sha256")
    .update(`EVAL_LOGIC_VERSION=${EVAL_LOGIC_VERSION}\n${getJobMatchingSkill()}`, "utf8")
    .digest("hex");
}
