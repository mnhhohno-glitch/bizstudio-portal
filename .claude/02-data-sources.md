...
## スカウト応募の3項目（応募日 / 配信日 / 経路=媒体）（T-101, 2026-06-23 追記）

| データ | 格納先 | フィールド | 備考 |
|--|--|--|--|
| 応募日 | portal `Candidate` 直持ち | `applicationDate`（`application_date`）| 求職者1人1件（T-091 手入力欄）。JST正午UTC（`+T12:00:00.000Z`）で保存 |
| 配信日 | portal `Candidate` 直持ち | `scoutDeliveryDate`（`scout_delivery_date`）| 同上。スカウト配信枠 `ScoutDeliverySlot.deliveryDate` とは別（こちらはCA手入力欄）|
| 経路（媒体名）| portal `Candidate` 直持ち | `mediaSource`（`media_source`）| マイナビ転職 / マイナビエージェント / indeed / 日経HR / 自社HP / dodaMaps 等。マイナビRPA・手動登録で記録 |
| 経路区分 | portal `Candidate` 直持ち | `applicationRoute`（`application_route`）| 「スカウト」/「応募」。⚠️ 画面Bの既存「経路」フィルタはこちら。T-101の一覧「経路」列は媒体名(`mediaSource`)の方 |

- **これら3項目は `Candidate` レコードに直接ある**ため、面談一覧・求職者管理一覧での表示は **join 不要**（`r.candidate.*` をそのまま読む）。
- `ScoutDeliverySlot`（`scout_delivery_slots`）は別概念のスカウト配信集計枠。`Candidate.scoutDeliverySlotId` で紐付くが、一覧3項目の表示には使わない。
- 表示・範囲比較は罠#17 に従い必ず `Asia/Tokyo` 基準（`toLocaleDateString("sv-SE",{timeZone:"Asia/Tokyo"})`）。詳細は `14-ui-component-map.md` の一覧画面マップ参照。
