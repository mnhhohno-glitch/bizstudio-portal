...
- **自動保存＋提出（T-069②）**：所感は**自動保存**（入力 2.5秒 debounce＋**日付移動時・離脱前(beforeunload)に即時保存**、`keepalive` fetch、`dirtyRef` で未編集時は送らない）。下書き保存ボタンは無し。**右上に「提出」ボタン**（`status=SUBMITTED`・`submittedAt`、提出済みは「✓ 提出済み」＋「再提出」）。**提出時のみ LINE WORKS 通知**（下書き自動保存では送らない）。POST の update は body に来たフィールドのみ反映（旧ドロワーの comment/aiBody を NULL で潰さない）。
