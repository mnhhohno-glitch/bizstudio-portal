...
## 52. 評価の指示文を変えるときは `EVAL_LOGIC_VERSION` を上げるか決める

**罠**: 求人評価の変更なしスキップ（`src/lib/eval-history.ts`）は、指示文の中身ではなく **判定の版（`logic_key` = `EVAL_LOGIC_VERSION` + SKILL.md 本文）** で「前回と同じ判定か」を見る（T-XXX step8）。指示文を変えても版を上げなければ、既存の評価はそのまま再利用される。

- **判定そのものが変わる変更**（評価ルールの基準・▲×の数え方・総合評価表・ランクの決め方）は `src/lib/analyze-bookmarks.ts` の `EVAL_LOGIC_VERSION` を上げる。上げると**全件が評価し直しになる**（費用が出るので件数と金額を先に出す）。
