...
| 項目 | AI側 | UI側 |
|--|--|--|
| 送付予定 | 「求人送付予定」（candidate-flags.ts 求人送付フラグ）| 「送付予定」「送付なし」（T-051 後）|
| 連絡手段 | （要確認、Step 2 で対応）| 「LINE WORKS」削除（T-051 後）|

**関連ケース**: T-051（面談入力フォーム ドロップダウン整理、DBマイグレーション「求人送付予定」31件含む50件を「送付予定」に統一）
