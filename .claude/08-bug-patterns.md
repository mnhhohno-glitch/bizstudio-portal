...
### M-2. マイページ回答通知が担当CAに届かない（宛先の対応表を社員情報と別に持っていた）

**症状**: 求職者がマイページで「気になる／応募したい」を送ったときの LINE WORKS 通知で、担当CA（奥村）に @メンションが付かない。

**真因（2026-09-26）**: 担当CAの宛先を環境変数 `LINEWORKS_ADVISOR_MAP`（CA名→LINE WORKS ID）から引いていた（箱B kyuujin からの移植）。表には6名（大野 将幸・大野 望・岡田 愛子・安藤 嘉富・南條 雄三・佐藤 葵）しか無く、支援中の担当CAのうち奥村 裕司・根岸 杏里沙・藤田 愛が未登録＝メンションなしでチャンネルに流れるだけだった。社員情報（User.lineworksId）には全員登録済み。さらに求職者サイトの応募・質問通知は `Employee.lineUserId`（LINE のID・全員未登録）で引いており、メンションが一度も付いていなかった。

**修正**: 宛先を `candidate.employeeId` → `User.lineworksId` に一本化（`src/lib/lineworks-ca-mention.ts`）。`LINEWORKS_ADVISOR_MAP` はコードから参照しない（env は残置）。見つからないときは大野 将幸へフォールバックし、本文末尾に注記。

**教訓**: 宛先の対応表を社員情報と別に持つと、人が増えたときに登録漏れで黙って届かなくなる。宛先は社員マスタ1か所から引き、引けないときは代表に「届いていない」と明記して送る。
