...
### 同日の他号機との重なり（`src/lib/scout-conditions/overlap.ts`・純関数）

- 編集モーダルを左右2列（最大幅 1560px）にし、右に「同日の他号機」パネル（`SameDayPanel.tsx`）。
...
- **重なり＝7軸すべてが交わる**（1軸でも交わらなければ重ならない）。境界は含む。「今日」は Asia/Tokyo。

| 軸 | 交わる条件 |
|--|--|
| 検索対象 | 同じ値 |
| 登録日 | 期間が交わる。「N日以内」は「今日−N日〜今日」に直してから比較。端が空（指定なし）は開区間 |
| 最終ログイン日 | 同上（N日以内は必ず今日を含むので実質つねに交わる） |
| 卒業年度 | 範囲が交わる（`15-26` と `21-22` は交わる／`15-17` と `21-22` は交わらない） |
| 経験社数 | 範囲が交わる（`--`=指定なし／`0社`=[0,0]／`～N社`=[0,N]／`7社以上`=[7,∞)。`～3社` と `～1社` は交わる） |
| 居住地 | 都道府県の集合が交わる。全国は何とでも交わる。東日本と西日本は交わらない。全国/東日本/西日本は都道府県に展開して比較（`expandAreaToPrefectures`。東京 と 神奈川 は交わらない） |
| 希望勤務地 | 都道府県の集合が交わる。指定なし（全国）は何とでも交わる |

- 関数: `isOverlapping(a, b, todayYmd)`（真偽）／`overlapAxes(a, b, todayYmd)`（軸ごとの内訳）／`findOverlapping(target, others, todayYmd)`／
  一覧用 `computeListOverlaps(rows, activeMachineIds, todayYmd)`。
- 見せ方: フォームを変えるたびにクライアントで判定し直し、重なる行は右パネルで赤い背景。1件以上重なればフォームの一番上に赤字1行
  `2号機 2-018、3号機 3-020 と重なっています`。**保存は止めない（警告のみ）**。
- 条件一覧: 有効・予約の条件で同日の他号機と重なるものに状態バッジの横に小さな赤い印「重なり」（ホバーで相手のレコード番号）。
  一覧 GET でサーバーが `attachListOverlaps` により `overlapRecordNos` を付ける（単件 POST/PATCH のレスポンスでは空配列）。
