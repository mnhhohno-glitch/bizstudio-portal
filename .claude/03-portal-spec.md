...
- ソート: `createdAt` 昇順。
- レスポンス `{ tasks: [{ id, title, status, createdAt(JST +09:00), fields:{"希望日時","面談形式","備考"}(無い値はnull・生テキスト), assignees:[{id,name}], candidateId(無ければnull) }] }`。
...
## 日程調整AIエージェント（T-139 step3・portal内蔵）

step1 の外部API 2本（上記）に加え、**判断する頭脳を portal 内部に実装**したもの。夜間バッチと外部受け口の2経路があり、
どちらも同じ共通コア（空き枠マッチング＋仮予約書き込み）を呼ぶ。

### 構成

| 要素 | パス | 役割 |
|--|--|--|
| 共通コア（枠探索） | `src/lib/schedule-agent/match-slot.ts` | 希望候補×対象CAカレンダーで空き枠を探す |
| 共通コア（仮予約） | `src/lib/schedule-agent/reserve.ts` | 共有「仮予約」カレンダーへイベント作成 |
| 夜間バッチ | `POST /api/internal/schedule-agent/run` | `x-api-key`=INTERNAL_API_KEY。mynavi_new タスクを自動処理 |
| 外部判定受け口 | `POST /api/external/schedule-agent/resolve` | `x-api-secret`=EXTERNAL_API_SECRET。メッセージ本文→日時抽出→仮予約→返信文面 |
| 定期実行 | `.github/workflows/schedule-agent.yml` | 30分毎 cron（稼働時間ガードはAPI側） |

### 枠ルール（将幸さん確定 Q1〜Q5）

- **Q1/Q3（枠長）**: 希望時間帯の**開始時刻から60分**を固定で取る。希望幅が60分より長くても短くても同じ（11:00〜17:00希望→11:00〜12:00 / 17:00〜17:30希望→17:00〜18:00）。
- **Q2（対象CA）**: env `SCHEDULE_AGENT_TARGET_USER_IDS`（カンマ区切り・**ハードコード禁止**）。大野将幸・安藤嘉富・奥村裕司の3名。バックヤード2名・産休2名は除外。
  - 3名のうち**誰か1人でも空いていれば「空き」**。複数空いていれば env リストの**先頭順**で1名を採用し、その氏名を仮予約イベントに記載（翌朝人が最終割り当て）。
  - フェイルセーフ: `getCalendarEvents` は未接続でも `[]`（＝終日空きに見える）を返すため、**GoogleCalendarConnection がある userId のみ判定対象**にしている。
- **枠の窓**: 開始 9:00〜20:00 / 当日不可（翌営業日以降）/ 翌営業日〜2週間以内 / 土日祝は不可（`isBusinessDay` 再利用）。
- **Q4（書き込み名義）**: env `SCHEDULE_RESERVATION_WRITER_USER_ID`（大野将幸）。そのユーザーのOAuth資格情報で共有カレンダーへ insert。
- **Q5（稼働時間帯）**: 平日 19:00〜翌9:00 ＋ 土日祝は終日。範囲外は `{skipped:true, reason:"business hours"}` で即終了（打刻連動はしない）。

### 夜間バッチの挙動

1. 稼働時間ガード（Q5）→ 範囲外なら skipped。`?force=true` でガードのみ外せる（手動疎通用。cron は付けない）。
2. 「日程調整」カテゴリ / `NOT_STARTED` / タイトルに「**新規面談調整**」を含む（mynavi_new）タスクを取得。
3. 同一応募者（candidateId 優先、無ければタイトルの `/新規応募者\s+(.+)$/` 氏名）の重複は**最新1件のみ**。
4. **先に `IN_PROGRESS` に更新してから**枠探索（二重予約防止）。
   - 確保成功 → 仮予約書き込み → `【日程調整AI】AI仮予約: {日時} で仮確定…` コメント → `COMPLETED`。
   - 確保不可 → `【日程調整AI】確保不可: …` コメントを**一度だけ**（既に「確保不可」コメントがあれば再掲しない）→ `NOT_STARTED` に戻す。
   - 例外・カレンダー書込失敗 → `NOT_STARTED` に戻す（`IN_PROGRESS` で放置しない＝次回再試行）。
5. 希望日時は**正規表現でパース**（LLM不要）。`第N希望: YYYY年M月D日（曜） HH:MM〜HH:MM` の定型。「第3希望: なし」は正規表現に一致せず自然にスキップ。

### 外部受け口の年付与ロジック（重要）

`messageBody` を Gemini に渡すが、**年（year）は LLM に出力させない**（ハルシネーション回避）。responseSchema で
`{ candidates: [{ month, day, startTime, endTime }] }` を強制し、**年はサーバー側で機械決定**する:
`executedAt` の年を基準に `baseYear → baseYear+1 → baseYear-1` の順で試し、「翌営業日〜2週間以内」の窓に収まる年を採用。
窓外の候補は捨てる。これにより **12月→1月跨ぎ**（12/28実行・1/5希望）も正しく解決される。

返信文面は env `SCHEDULE_AGENT_REPLY_TEMPLATE`（プレースホルダ `{日時}`）で差し替え可能。未設定時は定数の暫定文。
確保不可・日時抽出不可は `{ status: "unavailable" }`（RPAは返信しない区分）。

### 関連 env（本番設定が必要）

| env | 用途 | 未設定時 |
|--|--|--|
| `SCHEDULE_RESERVATION_CALENDAR_ID` | 共有「仮予約」カレンダーID（人手で作成・全CAに共有） | 両経路とも `{skipped:true, reason:"reservation calendar not configured"}` で安全終了 |
| `SCHEDULE_RESERVATION_WRITER_USER_ID` | 書き込み名義ユーザー（Q4・大野将幸の User.id） | 同上 |
| `SCHEDULE_AGENT_TARGET_USER_IDS` | 空き判定の対象CA（Q2・カンマ区切り・先頭順が採用優先） | `{skipped:true, reason:"no target CA configured"}` |
| `SCHEDULE_AGENT_REPLY_TEMPLATE` | 返信文面テンプレ（`{日時}` を差し込み） | 定数の暫定文を使用 |

### 仮予約カレンダーの運用

- 仮予約は**確定記録ではなく**、翌営業日に人が最終割り当てするまでの**仮置き場**。
- イベント: `【仮予約】{応募者名} / 担当候補:{採用CA名} / {採用した希望}（AI仮確定）`。説明欄に応募者名・面談形式・担当候補CA名・元タスクID。
- **AI は作成のみ。削除・更新はしない**（人が手動で削除する運用）。

### 既知の制限

- `getCalendarEvents` は**終日イベント（`start.date` のみ）を除外**するため、終日の予定（有給等）は枠のブロック要因にならない。
  仮予約は人が最終確認する前提のため許容している。
- `createCalendarEvent` は T-139 で第4引数 `calendarIdOverride?: string` を追加（**省略時は従来どおり接続ユーザー自身のカレンダー**＝既存呼び出しは挙動不変）。
