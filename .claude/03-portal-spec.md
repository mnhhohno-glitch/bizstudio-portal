...
| 有効が無くなった号機 | 通知なし | **「◯号機：本日（9/19）の配信条件がありません。配信は行われません」を LINE WORKS に1行**（タスクは作らない） |
...
- 実行は `activate.ts` の `runDateRollover(machineId)`（号機ロックの中で 2-1 有効の完了 → 2-2 期限切れ予約の完了 →
  2-3 当日の予約を有効化 → 2-4 通知 の順）。呼び口は2つ:
...
- **2-4 の通知は「状態遷移が起きた1回だけ」**（2-1 で有効を完了にしたのに 2-3 で上げられなかった呼び出しの中でだけ送る）。
  DB に LINE の送信記録は無いので、有効なしの状態で一覧を何度開いても 2回目以降は 2-1 の対象が無く再送されない。
  前日から有効が無かった号機（人が手で予約に戻した等）には 2-1 の遷移が無いため通知は出ない。
