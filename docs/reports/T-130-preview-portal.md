...
- コミット: **3f77f57**（master `e23fd4d..3f77f57`）＋本報告確定コミット。
- Railway デプロイ（本番サービス `bizstudio-portal`・env production）:
  - デプロイID **69a0dfee-1e16-4f50-9b73-1af5e76deb74** | **SUCCESS** | 2026-07-03 19:13:20 +09:00
  - ライブ確認: `POST https://bizstudio-portal-production.up.railway.app/api/candidates/probe/site-preview-url`
    （セッションなし）→ **HTTP 401 `{"error":"認証が必要です"}`**＝新ルートが本番稼働（未デプロイ時は404）。
