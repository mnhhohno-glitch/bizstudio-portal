...
## 4. 検証（実測・2026-07-03 デプロイ後 本番）

| # | 項目 | 結果 |
|---|---|---|
| 1 | 5008089 portal気になる = kyuujin気になる | portal CandidateJobResponse **4件**（7706,7718,7720,7728 INTERESTED）= kyuujin JobFeedback interested **4件** → **✓一致**（修正前7→修正後4） |
| 2 | 5008089 favorites GET | active BOOKMARK **15件**（sync archive=0）。**15維持が正**（9つのEXCLUDEDは重複導入で各社にアクティブ求人が生存＝ブックマークはアクティブ側に対応）。前レポートの「6件」は誤り |
| 3 | 5999999 往復（本番API） | (a) ON INTERESTED → HTTP200 `{success,updated}`・DB行1件 ✓／(b) OFF `response:null` → HTTP200 `{success,cleared,deletedCount:1}`・**DB行0＝即クリア** ✓ |
| 4 | 対象外ON→favorites消失→復帰 | Phase2 の突合ロジックは §3-b と同一コードパスで実データ検証済（archive=EXCLUDEDのみ・ガードで生存求人を保護）。DELETE handler はセッション必須のため UIからのE2Eは手動確認に委譲（コードパスは検証済） |
| 5 | 正常系の退行なし | WANT_TO_APPLY → HTTP200・DB反映 ✓／不正値 FOOBAR → **HTTP400維持** ✓（fail-closed不変） |
| 6 | バックフィル件数一致 | 3-a: dry-run 208 = 実削除 **208**／3-b: dry-run 297 = 実archive **297**。本番全体: CandidateJobResponse **853**（=1061−208）／archived_reason='job-excluded-sync' **297** |

### rollback CSV（verify/・コミット済）
- `verify/t128-stale-responses-rollback-2026-07-03.csv`（208行・削除レコード全列）
- `verify/t128-stale-responses-drift-2026-07-03.csv`（2行・削除せず報告）
- `verify/t128-excluded-bookmarks-rollback-2026-07-03.csv`（297行・archive対象）
- `verify/t128-excluded-bookmarks-ambiguous-2026-07-03.csv`（140行・触らず報告）

### ロールバック手順
- 3-a: rollback CSV の各行を `candidate_job_responses` に INSERT（id/candidate_id/external_job_id/response/responded_at/created_at で復元）。
- 3-b: rollback CSV の id について `UPDATE candidate_files SET archived_at=NULL, archived_reason=NULL WHERE id IN (...)`。

## 5. コミット / デプロイ

- コード＋スクリプト＋報告＋rollback CSV: コミット **5842128**（master `f83eafc..5842128`）。
- Railway 本番デプロイ（サービス `bizstudio-portal`・env production）:
  - デプロイID **3692d0e3-ae51-4027-8ba6-85eca965cc8c** | **SUCCESS** | 2026-07-03 20:22:24 +09:00
- バックフィルはデプロイ完了後、**railway ssh 経由**（in-container `node_modules/.bin/tsx`）で実行:
  - `scripts/t128-cleanup-stale-responses.ts --execute` → 208件削除
  - `scripts/t128-archive-excluded-bookmarks.ts --execute` → 297件archive
- 検証は本番URL `https://bizstudio-portal-production.up.railway.app/api/external/candidate-response` への実往復で確認（§4-3/5）。

### 申し送り（別途判断）
- **kyuujin側の対象外リアルタイム同期**: kyuujin→portal の EXCLUDED webhook が存在しないため、/v//site//kyuujin管理からの除外は portal に届かない。恒久対策は (A) kyuujin に EXCLUDED webhook を新設し portal に受信口を作る、(B) §3-b バックフィルの定期実行（cron）でリコンサイル、のいずれか。
- **ambiguous 140件**（重複導入で片方EXCLUDED・片方生存）は今回触っていない。重複求人自体の整理（kyuujin側のデータクレンジング）は別課題。

---

*本タスクは Phase1/2 実装 + Phase3 バックフィル（dry-run→rollback CSV→ID限定実行）を完了。unscoped な UPDATE/DELETE は使用していない。*
