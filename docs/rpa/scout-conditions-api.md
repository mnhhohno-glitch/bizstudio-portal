...
> **T-207（2026-09-17）追加**: `condition.template` に `templateNo`（`T-001` 形式のテンプレート番号）を追加しました。
> 既存キー（`templateId` / `name` / `subject` / `body`）の削除・改名はしていないため、**PAD 側を改修しなくても従来どおり動きます**。
> 件名・本文はポータルの「配信テンプレート」画面（`/scout/templates`）で管理する形になりました。集計ファイルの数式から取っている場合は、
> この API の `template.subject` / `template.body` に切り替えてください（差し込み記号は原文のままなので、置換処理は今のまま使えます）。
### 値の規約
...
| `template.subject` / `body` | その条件で使う**件名・本文**。差し込み記号（`[担当者]` `[社名]` `[最終学歴]` `[経験職種]`）は **展開せず原文のまま**返します。展開は RPA 側の1人ごとの処理で行ってください。**件名・本文のどちらにも4種類すべてが入り得ます**（T-207 でポータル管理に移行したため、本文の `[社名]` も置換対象です）。`template` 自体が null のこともあります（テンプレート未設定） |
| `template.templateNo` | **T-207 で追加**。ポータルの配信テンプレート番号（`T-001` 形式の通し番号）。どのテンプレートで配信したかを実績と突き合わせるための値です。読まなくても従来どおり動きます。まれに null（採番前） |
| `template.templateId` / `name` | 従来どおり。`templateId` は内部ID、`name` は画面上のテンプレート名 |
...
### 日付切替時の自動完了・自動有効化（T-209 → T-210 → T-211。RPA 側の対応は不要）

運用ルール: **有効になるのは「配信日が今日（JST）」の条件だけ**です。前日のうちに翌日分を予約しておけば、翌朝この口を
呼んだ時点で自動的に有効になります。配信日が過ぎたものは自動で完了になります。

この口を呼んだ時点で、ポータルはその号機について次の順に判定します（号機ごとに直列化）。

1. **前日以前の有効を自動で「完了」にする** — 有効（RUNNING）の条件の配信日が今日（JST）より前なら完了にします。
   配信日が空の有効は、最新の実行日時（JST の日付）が今日より前なら完了にします（実行実績も無ければそのまま）。
   これは**枯渇ではありません**（`is_dry` は立ちません。枯渇の判定＝送信10件未満・予約消化の順序は従来どおりです）。
2. **配信日が過ぎた予約を自動で「完了」にする** — 予約（QUEUED）の配信日が今日より前なら完了にします（通知は出ません）。
   配信日が空の予約は触りません。
3. **「配信日が今日」の予約を自動で「有効」にする** — 有効が無くなったら、**配信日が今日（JST）と一致する**予約のうち、
   ポータル一覧の ▲▼ の並び順で**一番上の1件**を有効にします。
   **配信日が今日の予約が無ければ何も上げません**（過去・未来・空欄の予約は上げません）。その場合は `condition: null` で返り、
   その号機はその日は配信されません。

**この切替そのものの LINE WORKS 通知は T-212 で廃止しました**（「日付切替により条件Aを完了 → 条件Bに切替」「本日の
配信条件がありません」の号機ごとの通知は出ません）。代わりに下の「朝のまとめ通知」を1通だけ送ります。

> T-210 は「配信日が今日**以前**・配信日が空」の予約も有効に上げていたため、配信日が過去の予約が走り出す／過去日付の
> 有効が何度も切り替わる状態になりました。T-211 で「今日ちょうど」に限定しています。

### 朝の「本日の配信条件」まとめ通知（T-212。RPA 側の対応は不要）

この口が **その日（JST）の 07:00 以降に最初に呼ばれたとき**、日付切替を終えたあとに、ポータルから LINE WORKS へ
**全号機分の本日の配信条件を1通にまとめて**送ります（**1日1回**。号機を問わず最初の呼び出しが送ります）。

- 07:00 より前の呼び出し（1号機の夜間フローが 5:00 に取りに来る分）では**送りません**。この時点でも日付切替そのものは走ります。
- 1通の中に、稼働中の号機を号機番号順に1行ずつ並べます。有効な条件が無い号機は「条件なし」と書きます。
- RPA が一度も動かない日は送られません。
- 通知の送信に失敗しても、この口のレスポンスは正常に返します（RPA は止まりません）。
...
送信済みかどうかはポータルの `scout_daily_notifications`（日付が主キー）で管理しています。複数の号機が同時に取りに来ても、
行を INSERT できた1回だけが送信します。

**レスポンスの形は従来どおりで、キーの追加・削除・改名はありません。** RPA は今までどおり `condition` を読んでください。
...
| `searchResultCount` | – | **T-206（新規・任意）** マイナビ検索結果ページの「検索結果：全 1299 件」の数字（母数）。画面の文字列のまま送ってよい（`1,299` / `1,299件` / `1299 件` はポータル側でカンマ・「件」・空白を落として数値にします）。数値に直せないときは記録を null にするだけで、結果送信は成功します。省略・null 可（未改修の RPA は従来どおり動きます）。毎回そのまま送ってください。ポータルの画面ではその条件の**初回（最も古い実行）の値**を「結果」として表示します（T-213。回数分の合算はしません） |
...
| `switched` | 次の予約（配信日が今日・▲▼ 順の先頭）に切り替えたか |
...
1. 実績を記録
2. `sentCount < 10` なら枯渇。条件を「枯渇」にし、その号機の予約のうち **配信日が今日（JST）のもの**の先頭（▲▼ 順）を
   「実行中」へ切替 → LINE WORKS に1行通知（T-211 で候補の絞り込みを日付切替と同じ規則に揃えました。しきい値 10 件は不変）
3. 切り替えられる予約が無ければ、枯渇した条件を「実行中」のまま残して配信は継続 → LINE WORKS 通知＋ポータルタスク作成（担当者へ）
