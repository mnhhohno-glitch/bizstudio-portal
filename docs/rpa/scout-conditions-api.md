...
### 予約の自動切替（T-209。RPA 側の対応は不要）

この口を呼んだ時点でその号機に RUNNING の条件が無い場合、ポータルは「配信日が当日（JST）以前になっている予約」を1件だけ自動で RUNNING にしてから返します（配信日が一番古いもの。同じ日なら並び順が上のもの）。配信日が翌日以降の予約や、配信日が空の予約は上げません。

レスポンスの形は従来どおりです（キーの追加・削除はありません）。RPA は今までどおり `condition` を読んでください。
