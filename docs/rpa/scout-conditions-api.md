...
### 日付切替時の自動完了・自動有効化（T-209 → T-210。RPA 側の対応は不要）

この口を呼んだ時点で、ポータルはその号機について次の順に判定します（号機ごとに直列化）。

1. **前日の有効を自動で「完了」にする** — 有効（RUNNING）の条件の配信日が今日（JST）より前なら完了にします。
   配信日が空の有効は、最新の実行日時（JST の日付）が今日より前なら完了にします（実行実績も無ければそのまま）。
   これは**枯渇ではありません**（`is_dry` は立ちません。枯渇の判定＝送信10件未満・予約消化の順序は従来どおりです）。
2. **予約の先頭を自動で「有効」にする** — 有効が無くなったら、**配信日が空、または今日（JST）以前**の予約（QUEUED）のうち、
   ポータル一覧の ▲▼ の並び順で**一番上の1件**を有効にします（T-209 は配信日の古い順でしたが、並び順を優先します）。
   配信日が明日以降の予約だけしか無ければ何も上げません（その場合は `condition: null` で返ります）。

1・2 が両方起きた（有効が入れ替わった）ときだけ、ポータルから LINE WORKS に「日付切替」を含む1行通知が出ます。
